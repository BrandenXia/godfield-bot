...
def build_curriculum_heuristic(
    ...
    *,
    ...
    if dark_cloud_ruleset:
        for slug, (attack, _element) in verified_dark_cloud_weapon_cards(snapshot).items():
            token = vocabulary.token_id("weapons", slug)
            attack_token_values[token] = attack
            dark_cloud_attack_tokens.add(token)
        for slug, (cost, _element) in verified_dark_cloud_miracles(snapshot).items():
            token = vocabulary.token_id("miracles", slug)
            attack_token_values[token] = 0
            attack_miracles[slug] = (0, cost)
            dark_cloud_attack_tokens.add(token)
        ...
        if batch.phases[environment] == 0:
            ...
            if batch.resource_curriculum:
                ...
                removable_curse = (
                    ...
                    or (
                        bool(dark_cloud_flags[environment, actor])
                ...
                if attack_candidates:
                    ...
                    opponent_dark_clouded = (
                        bool(dark_cloud_flags[environment, opponent])
                        if dark_cloud_flags is not None
                        else False
                    )
